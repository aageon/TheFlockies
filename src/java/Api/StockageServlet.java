 ...
 */
@WebServlet(name = "StockageServlet", urlPatterns = {"/api/stockageservlet"}, initParams = {
...
public class StockageServlet extends HttpServlet {
    ...
    public void init() throws ServletException {
        try {
            ...
            if (dastockage == null) {
                dastockage = new DAStockage(url, login, password, driver);
            }
...
//    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            List<Stockage> Stockages = dastockage.listStockage();
            
            Gson gson = new Gson();
            
            response.setContentType("application/json");
            
            response.getWriter().write(gson.toJson(Stockages));
        } catch (Exception e) {
